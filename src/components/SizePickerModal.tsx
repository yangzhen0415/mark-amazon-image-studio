...
export default function SizePickerModal({ currentSize, onSelect, onClose, allowAuto = true }: Props) {
  ...
  const buttonClass = (active: boolean) => {
    return `rounded-[var(--ios-radius-md)] px-3 py-2 text-sm transition-colors ${active
      ? 'bg-[hsl(var(--ios-blue-tint))] text-[hsl(var(--primary))] ring-2 ring-[hsl(var(--primary)/0.16)]'
      : 'bg-[hsl(var(--muted))] text-gray-600 hover:bg-[hsl(var(--surface-elevated))] dark:text-gray-300'
    }`
  }
  ...
  return (
    <Sheet onClose={onClose} rootClassName="z-[70]" className="max-w-md p-5 pt-8" labelledBy="size-sheet-title">
        ...
        <div className="space-y-6">
          <div className="ios-segmented flex w-full">
            {allowAuto && (
              <button
                onClick={() => setMode('auto')}
                data-active={mode === 'auto'}
                className={`ios-segment flex-1 py-1.5 text-sm font-medium ${mode === 'auto' ? 'text-gray-800 dark:text-gray-100' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
              >
                自动
              </button>
            )}
            <button
              ...
              data-active={mode === 'ratio'}
              className={`ios-segment flex-1 py-1.5 text-sm font-medium ${mode === 'ratio' ? 'text-gray-800 dark:text-gray-100' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
            >
              按比例
            </button>
            <button
              onClick={() => setMode('resolution')}
              data-active={mode === 'resolution'}
              className={`ios-segment flex-1 py-1.5 text-sm font-medium ${mode === 'resolution' ? 'text-gray-800 dark:text-gray-100' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200'}`}
            >
              自定义宽高
            </button>
          </div>
          ...
          <div className="h-[380px] max-h-[55vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-white/10 pr-1 -mr-1 pb-2">
                ...
                {ratio === 'custom' && (
                  <label className="block animate-fade-in">
                    ...
                    <input
                      ...
                      className={`ios-field w-full px-3 py-2 text-sm ${
                        customRatioValid
                          ? 'ios-field text-gray-700 dark:text-gray-200'
            ...
            {mode === 'resolution' && (
              <div className="space-y-5 animate-fade-in">
                  ...
                  <div className="flex items-center gap-4">
                    <label className="flex-1">
                      ...
                      <input
                        ...
                        className="ios-field w-full px-3 py-2 text-sm text-gray-700 dark:text-gray-200"
                        placeholder="例如 1024"
                      />
                    </label>
                ...
                <div className="ios-group p-3 text-xs text-gray-600 dark:text-gray-400">
        ...
        <div className="mt-5 flex gap-2">
          ...
          <button
            ...
            className="ios-button ios-button-filled flex-1 px-4 py-2.5 text-sm font-medium disabled:cursor-not-allowed disabled:opacity-50"
          >
            确定
          </button>
        </div>
