  ...
  return (
    <>
      ...
      {effectiveVisible && createPortal(
        <div
          ref={tooltipRef}
          className={`ios-floating-chrome fixed pointer-events-none px-3 py-2 text-xs font-normal text-foreground ${className}`}
          style={{
        ...
        >
          {children}
          <div
            className={`absolute left-0 border-4 border-transparent ${position?.placement === 'bottom' ? 'bottom-full border-b-[hsl(var(--surface-glass))]' : 'top-full border-t-[hsl(var(--surface-glass))]'}`}
            style={{
