...
const TASK_SIZE_PATTERN = /^\s*(\d+)\s*[xX×]\s*(\d+)\s*$/
const WIDE_PREVIEW_RATIO = 1.45

function parseTaskSizeRatio(size: string | undefined | null) {
  if (!size) return null
  const match = size.match(TASK_SIZE_PATTERN)
  ...
  useEffect(() => {
    ...
    const applyThumbnail = (thumbnail: { dataUrl: string; width?: number; height?: number }) => {
      ...
      if (thumbnail.width && thumbnail.height) {
        ...
        setCoverAspectRatio(thumbnail.width / thumbnail.height)
      }
    }
